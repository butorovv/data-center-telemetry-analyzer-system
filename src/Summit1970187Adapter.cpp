...
DetectorResult isolationForestResult(const PreparedData& data, const PrototypeRunOptions& options)
{
    ...
    params << "source=summit_1970187_adapter;trees=" << options.isolationTrees
...
DetectorResult Summit1970187Adapter::runKMeans(const PreparedData& data, const PrototypeRunOptions& options)
{
    ...
    std::ostringstream params;
    params << "source=summit_1970187_adapter;k=" << options.kmeansClusters
...
DetectorResult Summit1970187Adapter::runIsolationForest(const PreparedData& data, const PrototypeRunOptions& options)
{
    const auto started = std::chrono::steady_clock::now();
...
DetectorResult Summit1970187Adapter::runHybrid(
    ...
    const PrototypeRunOptions& options)
{
    ...
    result.executionMs = std::chrono::duration<double, std::milli>(finished - started).count();
    std::ostringstream params;
    params << "source=summit_1970187_adapter;base=isolation_forest;threshold=" << options.isolationThreshold;
    result.parameters = params.str();
    return result;
}
