...
9. Выполняет отдельную честную событийную валидацию на втором датасете 1970187.
...
Второй датасет не имеет сырых колонок вида `gpu0_core_temp`, `p0_power`. Вместо этого в нем есть агрегированные телеметрические признаки с разными окнами:

- `*_1min`;
- `*_5min`;
- `*_15min`;
- дополнительные долгие агрегаты `*_1h`, `*_6h`, которые в текущей событийной валидации не используются.

Для режима 1970187 признаки выбираются по окну, выбранному в GUI или CLI:

- `1min` - только числовые телеметрические признаки с суффиксом `_1min`;
- `5min` - только признаки с суффиксом `_5min`;
- `15min` - только признаки с суффиксом `_15min`.

Из feature vector явно исключаются `timestamp`, `hostname`, `GPU`, `is_failure`, serial/id/job-поля и другие идентификаторы. Поле `is_failure` используется только после работы алгоритма как ground truth.

Такой выбор сделан специально, чтобы не менять основной адаптер `SummitPrototypeAdapter` и не подмешивать проверочную метку в признаки.
...
```text
timestamp, hostname, GPU, is_failure, числовые телеметрические признаки выбранного окна *_1min / *_5min / *_15min
```

`is_failure == 1` означает реальный Double-Bit Error.

### 13.2 Алгоритм честной событийной валидации

1. CSV `points_with_jobs_tele_ult.csv` сканируется через `CsvReader`.
2. По выбранному окну (`1min`, `5min`, `15min`) формируется список числовых телеметрических признаков с соответствующим суффиксом.
3. Из признаков исключаются `is_failure`, `timestamp`, `hostname`, `GPU`, serial/id/job-поля и любые категориальные идентификаторы.
4. Для каждой валидной строки создается `TelemetryRow`; `is_failure` сохраняется только как проверочная метка `syntheticAnomaly`, а не как feature.
5. Строки с `is_failure == 1` собираются как DBE-события ground truth.
6. Данные нормализуются через `Preprocessor`.
7. Строится граф через `GraphBuilder`.
8. Запускается `Summit1970187Adapter::runHybrid` без принудительного выставления аномалий.
9. Для каждой DBE-строки проверяется только фактическая метка алгоритма `hybrid.labels[row]`.
10. Если алгоритм сам выставил `anomaly = 1`, событие считается положительным:

```text
positive = yes
algorithm_detected = yes
detection_time = failure_time - window_seconds
lead_time_sec = window_seconds
```

11. Если алгоритм не выставил `anomaly = 1`, событие остается отрицательным:

```text
positive = no
algorithm_detected = no
detection_time = -
lead_time_sec = -1
```

Окна дают такие значения lead time только для положительных случаев:
...
### 13.3 Что изменено относительно демонстрационной proxy-версии

Старая демонстрационная логика, где proxy-строка за 15 минут до сбоя могла вручную засчитываться как аномалия, отключена. В валидаторе больше нет логики вида:

```cpp
hybrid.labels[row] = 1;
hybrid.scores[row] = max(score, 1.0);
```

Агрегированная строка окна используется только как входное представление состояния системы. Она не считается обнаруженной аномалией автоматически.

`failures.csv` не является обязательным источником основной логики. Он используется только как optional sanity-check: приложение считает количество строк и выводит предупреждение, что ground truth берется из `points_with_jobs_tele_ult.csv`.

### 13.4 Проверенный результат честной версии

Команда:

```powershell
.\telemetry_analyzer_honest_validation.exe --validate-1970187 --points .\second_dataset\points_with_jobs_tele_ult.csv --failures .\second_dataset\failures.csv --threshold 0.75 --validation-window 15
```

Фактический результат после отключения forced anomaly:

```text
Validation window: 15min
Total rows: 50259
is_failure == 1: 295
Valid failure rows: 166
Skipped failure rows: 129
Processed events: 166
Positive: 0
Negative: 166
Detection rate: 0.0000
Lead time for positives: 900 sec
proxy_used: 0
Hybrid anomalies: 0
```

Это означает, что текущая честная проверка на выбранном пороге `0.75` не подтвердила раннее обнаружение DBE гибридным алгоритмом. Для РПЗ это нужно описывать как ретроспективную событийную проверку: из 166 пригодных DBE-событий алгоритм самостоятельно обнаружил 0 предсбойных агрегированных состояний. Положительный lead time больше не подставляется искусственно.
...
Команды проверки по окнам:

```powershell
.\telemetry_analyzer_honest_validation.exe --validate-1970187 --points .\second_dataset\points_with_jobs_tele_ult.csv --failures .\second_dataset\failures.csv --threshold 0.75 --validation-window 1
.\telemetry_analyzer_honest_validation.exe --validate-1970187 --points .\second_dataset\points_with_jobs_tele_ult.csv --failures .\second_dataset\failures.csv --threshold 0.75 --validation-window 5
.\telemetry_analyzer_honest_validation.exe --validate-1970187 --points .\second_dataset\points_with_jobs_tele_ult.csv --failures .\second_dataset\failures.csv --threshold 0.75 --validation-window 15
```

Фактическая сводка честной проверки:

| Окно | Всего строк | is_failure == 1 | Валидных DBE | Пропущено DBE | Positive | Negative | Detection rate | Lead time для positive |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| 1min | 50259 | 295 | 162 | 133 | 0 | 162 | 0.0000 | 60 сек |
| 5min | 50259 | 295 | 164 | 131 | 0 | 164 | 0.0000 | 300 сек |
| 15min | 50259 | 295 | 166 | 129 | 0 | 166 | 0.0000 | 900 сек |

Проверено:

- файл `points_with_jobs_tele_ult.csv` успешно прочитан;
- `is_failure` используется только как ground truth;
- признаки выбираются по суффиксам `_1min`, `_5min`, `_15min`;
- forced anomaly удалена;
- отрицательные случаи попадают в таблицу валидации как `positive = no`, `algorithm_detected = no`, `lead_time_sec = -1`;
- `failures.csv` не участвует в расчете, только выводится как optional sanity-check;
- GUI собирается, содержит выбор окна и отдельный экспорт валидации CSV.

Вывод для РПЗ: текущий результат является честной отрицательной валидацией на подготовленных агрегатах 1970187, а не доказательством раннего обнаружения DBE. Это нормально: количественная оценка качества алгоритмов остается в НИР/основной части, а 1970187 показывает механизм событийной проверки на реальном журнале отказов.

## 26. Ограничения и допущения

1. Второй датасет 1970187 содержит агрегированные признаки, а не полный сырой 10-секундный ряд.
2. Для 1970187 используются агрегированные признаки выбранного окна (`1min`, `5min`, `15min`); forced proxy-анomaly отключена.
...
Использовать разделы 23, 24 и 25. Включить команды запуска и честную сводку 1970187: positive/negative, detection rate, skipped reasons и lead time только для positive-событий.

### 27.9 Заключение

Сформулировать, что система реализует полный цикл анализа телеметрии, поддерживает несколько форматов данных, содержит GUI, БД, алгоритмы и ретроспективную событийную проверку DBE без forced anomaly.
...
| Lead time > 0 | в честной проверке на текущем пороге не подтверждено; positive=0, negative>0 |
| Документация | обновлена |

## 30. Итог

Проект представляет собой законченную учебную систему анализа телеметрии. В нем есть загрузка разных форматов, предобработка, база данных, граф соседства, три алгоритма обнаружения аномалий, Qt GUI, Docker-инфраструктура и отдельная честная событийная валидация на втором датасете 1970187.
